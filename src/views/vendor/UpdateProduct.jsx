...
const UpdateProduct = () => {
    ...
    const fetchProductDetails = async () => {
        try {
            const response = await apiInstance.get(`vendor-update-product/${userData?.vendor_id}/${pid}/`)
            console.log('product details fetched: ', response.data)
            setProduct(response.data)
    
            //note: the following properties are named in their respective singular forms
    ...
    const handleProductFileChange = (event) => {
            event.preventDefault()
            const file = event.target.files[0];
            console.log('image file has a type of: ', typeof(file))
            if (file){
    ...
    const handleSubmit = async (event) => {
        ...
        try{
            ...
            const response = await apiInstance.patch(`vendor-update-product/${userData?.vendor_id}/${pid}/`, formData, {
                headers: {
            ...
            Toast.fire({
                ...
                text: 'An error occurred while updating product. Please try again.',
            })
        } finally {
            setIsLoading(false)
        }
                                    ...
                                    <div>
                                        <div className="row text-dark">
                                            <div className="col-lg-6 mb-2">
                                                ...
                                                name="image"
                                                id="image"
                                                onChange={(event) => handleProductFileChange(event) || product.image}
                                                />
                                            </div>
                                            <div className="col-lg-6 mb-2 ">
                                    ...
                                    {gallery.map((galItem, index) => (
                                        <div key={index} className="row text-dark">
                                            <div className="col-lg-4 mb-2 content-center">
                                                ...
                                                {galItem.image && (
                                                    <img
                                                    src={galItem.image.preview? galItem.image.preview : galItem.image}
                                                    style={{ width: "100%", height: "250px", objectFit: "cover" }}
                                                    alt=""
                                                    />
                                                )}
